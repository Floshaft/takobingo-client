...
export default function GamePage({ onNavigate, room: initialRoom, player }) {
  ...
  const [scores, setScores]       = useState(
    initialRoom?.players?.map(p => ({ ...p, score: 0 })) || []
  )
  ...
  // hostId fiable depuis le serveur
  const isHost = room?.hostId === socket?.id

  useEffect(() => {
    if (!socket) return

    // Carton reçu du serveur
    ...
    socket.on('chat:reaction', (m) => addChat({ ...m, text: m.emoji }))
  ...
  const handleMarkCell = async (idx) => {
    ...
    if (!drawnItems.includes(word)) return
    try {
  ...
  const handleBingo = async () => {
    setBingoError('')
    try { await emitAsync(socket, 'game:bingo', {}) }
    catch (e) {
      ...
      <main className="game-main">
        ...
        <div className="game-topbar">
          <div className="current-ball-wrap">
            <div className="current-ball" style={lastItem ? {
              background: `linear-gradient(135deg, ${ballColor}, ${ballColor}99)`,
              border: 'none', boxShadow: `0 0 16px ${ballColor}60`
            } : {}}>
              {drawnItems.length > 0 ? drawnItems.length : '?'}
            </div>
            <div>
              <div className="current-word">{lastItem ? `#${drawnItems.length} — ${lastItem}` : 'En attente du tirage...'}</div>
              <div className="current-sub">
                {isHost
                  ? drawnItems.length === 0 ? '👑 Tu es l\'hôte — tire la première boule !' : `${drawnItems.length} / ${totalItems} boules tirées`
                  : 'En attente que l\'hôte tire une boule...'}
              </div>
            </div>
          </div>
        ...
        <div className="card-wrap">
          {!cardReady ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', color: '#9090b8' }}>
              <div style={{ fontSize: '2rem', animation: 'takoFloat 1.5s ease-in-out infinite' }}>🐙</div>
              <div style={{ fontWeight: 700 }}>Préparation de ton carton...</div>
            </div>
          ) : (
            <div style={{ width: '100%', maxWidth: '320px' }}>
              ...
              <div className="card-legend">
                <span className="legend-dot" style={{ background: '#6BCB77' }} /> Disponible
